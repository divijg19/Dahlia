...
class DeploymentManager:
    ...
    def build_application(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                self._log_action("BUILD", "SUCCESS", "Go application built successfully")
                return True
            else:
    ...
    def build_docker_image(self) -> bool:
        ...
        self._log_action("DOCKER_BUILD", "STARTING", f"Building Docker image: {image_name}")
        ...
        try:
            ...
            if result.returncode == 0:
                self._log_action("DOCKER_BUILD", "SUCCESS", f"Docker image {image_name} built")
                return True
            else:
                self._log_action("DOCKER_BUILD", "FAILED", f"Docker build failed: {result.stderr}")
                return False
    ...
    def deploy_container(self, environment: str = "development") -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                self._log_action("DEPLOY", "SUCCESS", f"Container deployed: {container_name}")
                return True
            else:
    ...
    def health_check(self, environment: str = "development", retries: int = 3) -> bool:
        ...
        if not env_config:
            self._log_action("HEALTH_CHECK", "ERROR", f"Unknown environment: {environment}")
            return False
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("status") == "healthy":
                        self._log_action("HEALTH_CHECK", "SUCCESS", 
                                       f"Application healthy at {url}")
                        return True
                
                self._log_action("HEALTH_CHECK", "RETRY", 
...
def main():
    ...
    parser.add_argument('--action', choices=['build', 'docker', 'deploy', 'health', 'full'],
